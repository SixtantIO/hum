...
(defn read-bits [^BigInteger i field-lengths]
  (let [binary (.toString i 2)]
    (map
      (fn [[start end]]
        (BigInteger. (subs binary start end) 2))
      (partition 2 1 (reductions + field-lengths)))))
...
(defrecord Diff [bid?             ; first bit of lots; 0 if ask, 1 if bid
  ...
  ser/DTO
  (serialize [this out]
    (let [tbits (max (.bitLength ^BigInteger ticks) 1)
          qbits (max (.bitLength ^BigInteger lots) 1)]
      (b/encode
        (uints
          tbits                       ; price
          1                           ; bid?
          qbits                       ; qty
          (:pbits-bits *diff-codec*)  ; # of pbits in this msg
          (:qbits-bits *diff-codec*)) ; # of qbits in this msg
        out
        [ticks bid? lots (biginteger tbits) (biginteger qbits)])))

  (deserialize [this in]
    (let [in-bytes (mapv b/byte->ubyte (.readAllBytes ^InputStream in))
          as-int (uint/ubytes->uint in-bytes)
          [as-int qbits] (uint/unpack-two-ints as-int (:qbits-bits *diff-codec*))
          [as-int pbits] (uint/unpack-two-ints as-int (:pbits-bits *diff-codec*))
          [as-int lots] (uint/unpack-two-ints as-int qbits)
          [as-int bid?] (uint/unpack-two-ints as-int 1)
          [as-int ticks] (uint/unpack-two-ints as-int pbits)]
      (assoc this
        :bid? bid?
        :ticks ticks
        :lots lots))))
...
(defn assert-bijective! [dto dto-bytes]
  (let [deserialized (try
                       (ser/deserialize dto (ByteArrayInputStream. dto-bytes))
                       (catch Exception e
