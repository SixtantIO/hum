...
(def ^:dynamic *rem-codec* nil) ; changes as the price+qty field bit lengths change
...
(def ^:const SNAPSHOT  0) ;; Order book snapshot
(def ^:const TIMESTAMP 1) ;; Timestamp 'snapshot' message -- all other
                          ;; timestamps are offsets from this message type.
(def ^:const DIFF      2) ;; Book level diff
(def ^:const REMOVAL   3) ;; Book level removed
...
(defn ticks+side-codec [pbits]
  (uints
    1  ;; Single bit for the side
    pbits)) ;; Then the # of ticks (price)


(defn >rem-codec [snapshot-dto]
  (ticks+side-codec (:pbits snapshot-dto)))


(defrecord Removal [bid? ticks]
  ser/DTO
  (serialize [this out]
    (b/encode *rem-codec* out [bid? ticks]))

  (deserialize [this in]
    (let [[bid? ticks] (b/decode *rem-codec* in)]
      (assoc this :bid? bid? :ticks ticks))))
...
(extend-protocol DTOTyped
  ...
  Removal (get-type-flag [this] REMOVAL)
...
(defmethod message->dto REMOVAL [{:keys [price bid?]}]
  (map->Removal
    {:bid? (if bid? BigInteger/ONE BigInteger/ZERO)
     :ticks (biginteger (->ticks price *tick-size*))}))


(defmethod dto->message REMOVAL [{:keys [bid? ticks]}]
  (messages/order-book-diff
    {:price (* (bigdec ticks) *tick-size*)
     :qty BigDecimal/ZERO
     :bid? (pos? bid?)}))
...
(defrecord L2Writer [^OutputStream out tick-size lot-size diff-codec rem-codec timestamp]
  Closeable
  (close [this]
    (.flush out)
    (.close out))
      ...
      (binding [*tick-size* @tick-size
        ...
        (let [msg-ts (:timestamp message)
          ...
          (when (= (int (get-type-flag dto)) SNAPSHOT)
            ...
            (vreset! rem-codec (>rem-codec dto)))
...
(defrecord L2Reader [^InputStream in tick-size lot-size diff-codec rem-codec timestamp]
  Closeable
  (close [this]
    (.close in))
...
(defn writer [out]
  (->L2Writer out (volatile! nil) (volatile! nil) (volatile! nil) (volatile! {}) (volatile! 0)))

(defn reader [in]
  (->L2Reader in (volatile! nil) (volatile! nil) (volatile! nil) (volatile! {}) (volatile! 0)))
